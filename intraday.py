...
# MODEL PERSISTENCE
# -------------------------
def model_filepath(symbol):
    safe = symbol.replace("/", "_")
    return os.path.join(MODEL_DIR, f"{safe}_rf.pkl")

def save_model(symbol, model):
    try:
        path = model_filepath(symbol)
        joblib.dump(model, path)
        logging.info(f"Model saved for {symbol} -> {path}")
    except Exception as e:
        logging.error(f"Failed to save model for {symbol}: {e}")
...
def load_model(symbol):
    try:
        ...
        if os.path.exists(path):
            ...
            logging.info(f"Loaded model for {symbol} from {path}")
            return model
        logging.debug(f"No model file for {symbol} at {path}")
    except Exception as e:
...
def analyze_market_conditions(symbol, df):
    try:
        ...
        importance = None
        try:
            # Try to get feature importances out of the pipeline (if rf is the last step)
            ...
            logging.debug(f"Could not log feature importances for {symbol}: {e}")
...
def train_rf_model(symbol, df, retries=5):
    """
    Train a RandomForestClassifier inside a pipeline with StandardScaler.
    Saves the trained model to disk and returns the fitted pipeline.
    """
    ...
    for attempt in range(retries):
        try:
            ...
            df_no_trade = df[df['target'] == 0]

            # balance classes by sampling no-trade rows
            try:
                desired_no_trade = max(1, min(len(df_buy), len(df_sell)) * 2)
                df_no_trade = df_no_trade.sample(n=min(len(df_no_trade), desired_no_trade), random_state=42, replace=True)
            except Exception:
                df_no_trade = df_no_trade.sample(n=min(100, len(df_no_trade)), random_state=42, replace=True) if len(df_no_trade) > 0 else df_no_trade

            df_train = pd.concat([df_buy, df_sell, df_no_trade])
            ...
            if len(df_train) < 30:
                logging.warning(f"Insufficient data to train RF model for {symbol}: {len(df_train)} rows")
                time.sleep(10)
                continue
            ...
            # Use a pipeline with scaler so production predictions are consistent
            pipeline = Pipeline([
...
def predict_next_price(df, symbol, dynamic_threshold):
    try:
        ...
        try:
            ...
            logging.debug(f"Could not compute feature importance for {symbol}: {e}")
...
def save_bot_data(symbol, analysis_data, account_info, open_trades):
    try:
        data = {
            ...
            'symbol': symbol,
            'analysis': {
                'predicted_price': safe_format(analysis_data.get('predicted_price'), '.5f') if analysis_data else 'N/A',
                'signal': analysis_data.get('signal', 'N/A') if analysis_data else 'N/A',
                'dynamic_threshold': safe_format(analysis_data.get('dynamic_threshold'), '.4%') if analysis_data else 'N/A',
                'confidence': safe_format(analysis_data.get('confidence'), '.2f') if analysis_data else 'N/A',
                'rsi': safe_format(analysis_data.get('rsi'), '.2f') if analysis_data else 'N/A',
                'atr': safe_format(analysis_data.get('atr'), '.5f') if analysis_data else 'N/A',
                'adx': safe_format(analysis_data.get('adx'), '.2f') if analysis_data else 'N/A',
                'vol': safe_format(analysis_data.get('vol'), '.6f') if analysis_data else 'N/A',
                'stoch_k': safe_format(analysis_data.get('stoch_k'), '.2f') if analysis_data else 'N/A',
                'stoch_d': safe_format(analysis_data.get('stoch_d'), '.2f') if analysis_data else 'N/A',
                'stoch_signal': analysis_data.get('stoch_signal', 'N/A') if analysis_data else 'N/A'
            },
            'account': {
...
def main():
    ...
    # Attempt to load existing models first to avoid unnecessary retraining
    for symbol in SYMBOLS:
        RF_MODELS[symbol] = load_model(symbol)
    ...
    for symbol in SYMBOLS:
        ...
        # If model not loaded, try to train it using historical data
        if RF_MODELS.get(symbol) is None:
